  ...
  async.waterfall([
    ...
    function(token, forgetfulUser, done) {
      ...
      var message = {
        ...
        html: '<h2>Forgot your ArcSavvy password?</h2><p>Reset your password here <a href="http://localhost:4200/passwordreset/' + token + '">Reset Password</a><br></p>'
      };
      mailgun.messages().send(message, function (err, body){
        if (err){
...
exports.logoutUser = (req, res) => {
  user.findOne({
    ...
    var index = oldList.indexOf(req.headers.authorization);
    if (index > -1) {
